...
module.exports = grammar({
  ...
  conflicts: ($) => [
    ...
    [$.ui_if_statement, $._statement], // ui_if_statement 与 _statement 的冲突
  ],
  ...
  rules: {
    ...
    component_body: ($) =>
      seq(
        "{",
        repeat(
          choice($.property_declaration, $.method_declaration, $.build_method),
        ),
        "}",
      ),
    ...
    ui_element_with_modifiers: ($) =>
      prec.right(
        15,
        seq($.ui_component, optional($.modifier_chain_expression)),
      ),
    ...
    ui_component: ($) =>
      prec.right(
        ...
        choice(
          ...
          // 特殊容器项
          seq(
            choice("ListItem", "GridItem", "ListItemGroup"),
            "(",
            optional($.component_parameters),
            ")",
            optional($.container_content_body),
    ...
    parenthesized_type: ($) =>
      prec.dynamic(1, seq("(", $.type_annotation, ")")),
    ...
    ui_if_statement: ($) =>
      seq(
        ...
        "{",
        repeat(
          choice($.arkts_ui_element, $.ui_control_flow, $.expression_statement),
        ),
        "}",
        optional(
          choice(
    ...
    _statement: ($) =>
      choice(
        ...
        // UI特定语句 - 提高优先级，避免被expression_statement误匹配
        $.ui_control_flow,
        prec(1, $.arkts_ui_element),
    ...
    ui_arrow_function_body: ($) => seq("{", repeat($._statement), "}"),
    ...
    builder_function_body: ($) => prec(3, seq("{", repeat($._statement), "}")),
    ...
    // 注意：$.comment 不需要显式匹配，因为它已在 extras 中定义（会被自动跳过）
    extend_function_body: ($) => prec(2, seq("{", repeat($._statement), "}")),
